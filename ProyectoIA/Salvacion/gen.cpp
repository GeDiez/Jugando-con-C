...
void decoder (double * x, int * genotipo)
{
    int i;
    *x = 0.0;
...
double fitness (double p1)
{
    return abs((p1-5)/(2+sin(p1)));
}
...
Individuo generarIndividuo (void){
    ...
    decoder(&x, ind.genotipo);
    ind.aptitud = fitness(x);
...
void cruzarSeleccion (Individuo * seleccion)
{
    ...
    for(i=0; i<POBLACION-1; i+=2)
    {
        if((double) rand()/(RAND_MAX+1.0) < PROB_CRUCE)
        {
            ...
            decoder(&x, seleccion[i].genotipo);
            seleccion[i].aptitud = fitness(x);
 
            decoder(&x, seleccion[i+1].genotipo);
            seleccion[i+1].aptitud = fitness(x);
        }
    }
}
